...
- **Secrets prod (terpasang)**: `DUITKU_MERCHANT_CODE`, `DUITKU_MERCHANT_KEY`, `DUITKU_ENV=production`, `JWT_SECRET`, **`GROQ_API_KEY`**, **`OPENROUTER_API_KEY`**, **`FONNTE_TOKEN`** (device token, BKF-19 refresh), **`FONNTE_WEBHOOK_SECRET`** (shared secret URL webhook, BKF-19), **`CLERK_SECRET_KEY`** (sk_live), **`CLERK_PUBLISHABLE_KEY`** (pk_live), **`CLERK_ISSUER`** (`https://clerk.barberkas-foundry.biz.id`) (BKF-14→15 production)
...
- **Last Updated**: 2026-07-09 (BKF-19: **Jalur WA ASLI via Fonnte — DEPLOYED ke production**. Webhook publik `/webhooks/fonnte` hardened: (1) verifikasi `?secret=` constant-time (`FONNTE_WEBHOOK_SECRET`, fail-closed di prod), (2) tenant HANYA via tabel `tenant_wa_devices` dari nomor device PENERIMA (migration `0009` ter-apply di D1 prod; device `6281558098096` → `t_cutoclock`), device asing → 404 tanpa fallback, (3) idempotency `inboxid` → `wa_webhook_events` UNIQUE (retry Fonnte = duplicate skip, anti booking dobel), (4) reuse `runReceptionist` yang SAMA dengan simulator. Bukti prod: secret salah → 403, device asing → 404, happy path FSM balas greeting Cut O'Clock + retry → duplicate. Catatan: kirim balasan WA nyata masih terblokir paket free Fonnte (`reply_error: "invalid message request on free package"`) — FSM & booking sudah jalan penuh, tinggal upgrade/aktivasi paket Fonnte.)
- **Prev**: 2026-07-08 (BKF-18: **Hardening jalur WRITE + security audit log — DEPLOYED ke production**. Audit semua endpoint tulis: tenant SELALU dipaksa dari sesi server (Clerk), input client (`?tenant=`/body `tenant_id`/`x-tenant-id`) tidak dipercaya — mismatch → 403 eksplisit + tercatat ke `security_audit_log` (migration `0008` ter-apply di D1 prod). Cakupan: `outcome.ts` (trustedTenantId intake/checkout), `retention.ts` (non-admin dikunci ke tenant sesi), `api.ts` (referensi lintas tenant capster/customer/service → 400; update booking cek changes → 404 jujur; validasi agent type → 400), `subscriptions.ts` (subscribe/cancel/upsell-respond mismatch → 403 + audit), `webhooks.ts` (fallback 'tenant pertama' Fonnte DIBLOKIR di production → 404 jujur + audit; test-send wajib tenant valid + jejak `wa_messages`). Bukti prod: `/` 200, endpoint API anon → 401 semua.)
- **Prev**: 2026-07-05 (BKF-16: **Tambal lubang auth + self-service tenant onboarding** — admin-gate `outcome/orders|:id|:id/proof|telemetry/delivery` + gerbang `/webhooks/fonnte/test-send`; subscriptions tenant-scope ditegakkan (scope() + ownership 403); `POST/GET /api/v1/auth/tenants` = onboarding tenant 1-request (tenant + 5 layanan default + capsters + map owner email, validasi subdomain + reserved + idempotent 409); frontend `oapi`/`aapi` bawa Bearer token + tab **Admin** di dashboard (form onboarding + daftar tenant). Test lokal: auth off → terbuka jujur; dev-bypass owner → 403 di semua endpoint admin, subs tenant lain 403, body-injection `tenant_id` ter-scope, capability URL tetap public.)
...
- ✅ ~~Integrasi WA nyata (Fonnte)~~ — BKF-19: webhook inbound + FSM jalan penuh di prod. Sisa: paket Fonnte free memblokir KIRIM balasan (`invalid message request on free package`) — perlu upgrade paket / aktivasi device di dashboard Fonnte supaya balasan bot benar-benar terkirim ke WA customer.
...
3. ✅ ~~Integrasi Fonnte webhook `/webhooks/fonnte`~~ (BKF-19: DONE — inbound + FSM live di prod; sisa upgrade paket Fonnte utk outbound reply). Set URL webhook di dashboard Fonnte: `https://barberkas-aaas.pages.dev/webhooks/fonnte?secret=<FONNTE_WEBHOOK_SECRET>`.
